...
###########################################################################################
# read GD data file 处理GD指标
dat = pd.read_csv("./num_res/{}.GD.csv".format(market))

# split into two experiments 前6个一组 后5个一组
exp1_GD = dat[dat.columns[:7]]
# exp2_GD = dat[dat.columns[6:]]

# calculate statistics 计算3个 最佳 均值 标准差
...
# find best and second best algorithm 取均值的最好两个
...
###########################################################################################
# read Spacing data file 处理S指标
dat = pd.read_csv("./num_res/{}.Spacing.csv".format(market))

# split into two experiments
exp1_Spacing = dat[dat.columns[:7]]
# exp2_Spacing = dat[dat.columns[6:]]
...
###########################################################################################
# read MaxSpread data file 处理MS指标
dat = pd.read_csv("./num_res/{}.MaxSpread.csv".format(market))

# split into two experiments
exp1_MaxSpread = dat[dat.columns[:7]]
# exp2_MaxSpread = dat[dat.columns[6:]]
...
###########################################################################################
# read Delta data file 处理delta指标
dat = pd.read_csv("./num_res/{}.Delta.csv".format(market))

# split into two experiments
exp1_Delta = dat[dat.columns[:7]]
# exp2_Delta = dat[dat.columns[6:]]
...
###########################################################################################
# read IGD data file 处理IGD指标
dat = pd.read_csv("./num_res/{}.IGD.csv".format(market))

# split into two experiments
exp1_IGD = dat[dat.columns[:7]]
# exp2_IGD = dat[dat.columns[6:]]
...
###########################################################################################
# read Hypervolume data file 处理HV指标
dat = pd.read_csv("./num_res/{}.Hypervolume.csv".format(market))

# split into two experiments
exp1_Hypervolume = dat[dat.columns[:7]]
# exp2_Hypervolume = dat[dat.columns[6:]]
...
# 开始输出详细数据
print("{}\n----------------------------------------------------------------------------------".format(market))
pd.options.display.float_format = '{:.2e}'.format  # 控制数据精度
pd.set_option('display.width', 1000)  # 设置全部显示内容
pd.set_option('display.max_columns', 1000)
pd.set_option('display.max_colwidth', 1000)
# stat1_overall stat2_overall 两个实验的统计汇总
